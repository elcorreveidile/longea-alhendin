...
import { listTerms, listSubjects, listGroups, addGroup, deleteGroup } from "@/db/docencia";
...
function scheduleText(schedule: unknown): string {
  ...
  return (schedule as { weekdays?: string[]; start?: string; end?: string }[])
    .map((b) => `${(b.weekdays ?? []).join("")} ${b.start ?? ""}–${b.end ?? ""}`)
    .join(" · ");
}
                ...
                <tbody>
                  {groups.map((g) => (
                    <tr key={g.id} className="border-b border-slate-100">
                      ...
                      <td className="px-3 py-2 text-slate-500">{scheduleText(g.schedule)}</td>
